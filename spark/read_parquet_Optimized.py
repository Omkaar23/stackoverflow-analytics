...
df = spark.read.parquet('s3a://stack-overflow-parquet-files/Posts.parquet/').select('_Id','_ParentId','_Score','_PostTypeId', h.unescape('_Title'), h.unescape('_Body'), read_tags('_Tags').alias('_TagsList'), '_AcceptedAnswerId','_AnswerCount','_OwnerUserId','_ViewCount')
df.show()
#df.createOrReplaceTempView("pF")

df1 = df.select('_Id', explode('_TagsList').alias('_PostTag'), '_Score')
df1.withColumnRenamed('_Id', '_PostId').collect()

#df1.withColumnRenamed('_Score', '_PostScore')
df1.show()
...
df2.withColumnRenamed('_Id', '_PostIdTag')
...
df_Tags_avg = spark.sql("SELECT pF3._PostTag as _AvgTag, CEIL(AVG(pF3._PostScore)) as _AvgScore from pF3 Group By pF3._PostTag Order By _AvgScore ASC")
df_Tags_avg.show(df_Tags_avg.count(), False)
...
df5 = spark.sql("""SELECT pF3._PostId, pF3._PostTag, pF3._PostScore  from pF3 where pF3._PostTag = 'java' and pF3._PostScore < (SELECT pFT._AvgScore from pFT where pFT._AvgTag = 'java') LIMIT 100  """)
df5.show()
